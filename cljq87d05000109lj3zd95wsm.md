...
tags: php, validation, symfony, php8
...
O porém é que esta exceção é padrão `ValidationFailedException` e como estamos construíndo uma API, é necessária uma resposta em formato de json.
...
No controller acima é possível ver que não usamos o tradicional `Request` do HttpFoundation e sim a nossa classe `CreateTransactionRequest` como dependência e é ai que ocorre a mágica, pois todas as dependências necessárias serão injetadas e a validação será realizada.
